...
function bible_verse_of_the_day($showlink, $language='niv') 
{
	...
	{
		$url = 'http://dailyverses.net/get/verse?language=' . $language . '&date=' . $bibleVerseOfTheDay_currentDate . '&url=' . $_SERVER['HTTP_HOST'] . '&type=daily2_7_3';
...
function random_bible_verse($showlink, $language='niv') 
{
	...
	{
		$url = 'http://dailyverses.net/get/random?language=' . $language . '&position=' . $position . '&url=' . $_SERVER['HTTP_HOST'] . '&type=random2_7_3';
		$result = wp_remote_get($url);
...
function get_language_url($language)
{
	...
	else if($language == 'es' || $language == 'nvi' || $language == 'rvr95' || $language == 'rvr60')
	{
		return '/es';
	}
